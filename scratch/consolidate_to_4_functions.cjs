...
// Clear existing api directory completely
...
// 1. auth.js
const authCode = `module.exports = (req, res) => {
...
fs.writeFileSync(path.join(apiDir, 'auth.js'), authCode, 'utf8');
...
// 3. public.js
const publicCode = `let publicState = {
    aparatur: [
...
// 4. admin-api.js
const adminApiCode = `let adminState = {
    aparatur: [
        { id: 1, nama: 'Hj. Andi Hasnani, S.Sos', nip: '19700101 199003 2 001', jabatan: 'Lurah Lompoe', foto: null, is_lurah: 1, sambutan: 'Selamat Datang di Website Resmi Kelurahan Lompoe, Kecamatan Bacukiki, Kota Parepare. Website ini hadir sebagai wujud transparansi publik dan kemudahan pelayanan administrasi bagi seluruh warga.', urutan: 1 },
        { id: 2, nama: 'Muhammad Amir, S.STP', nip: '19850512 200801 1 002', jabatan: 'Sekretaris Kelurahan', foto: null, is_lurah: 0, sambutan: '', urutan: 2 },
        { id: 3, nama: 'Siti Rahmah, S.E', nip: '19880920 201101 2 003', jabatan: 'Kasi Pelayanan Umum & Kesejahteraan', foto: null, is_lurah: 0, sambutan: '', urutan: 3 },
...
module.exports = (req, res) => {
    ...
    if (url.includes('aparatur')) {
        if (req.method === 'POST') {
            const newItem = { id: Date.now(), nama: body.nama || 'Aparatur Baru', nip: body.nip || '-', jabatan: body.jabatan || 'Staf', is_lurah: body.is_lurah ? 1 : 0, sambutan: body.sambutan || '', urutan: body.urutan || 5, foto: null };
    ...
    if (url.includes('pkk-wilayah')) {
        if (req.method === 'POST') {
            const newItem = { id: Date.now(), nama_wilayah: body.nama_wilayah || \`RW 0\${adminState.pkk.length + 1}\`, pkk_rw: body.pkk_rw || adminState.pkk.length + 1, pkk_rt: body.pkk_rt || 2, dasa_wisma: body.dasa_wisma || 5, krt: body.krt || 250, kk: body.kk || 300, pria: body.pria || 600, wanita: body.wanita || 600 };
    ...
    if (url.includes('berita')) {
        if (req.method === 'POST') {
            const newItem = { id: Date.now(), judul: body.judul || 'Berita Kelurahan Baru', ringkasan: body.ringkasan || '', isi: body.isi || '', tanggal: new Date().toISOString().split('T')[0], gambar: null };
    ...
    if (url.includes('sarana')) {
        if (req.method === 'POST') {
            const newItem = { id: Date.now(), nama_sarana: body.nama_sarana || 'Sarana Baru', kategori: body.kategori || 'Umum', lokasi: body.lokasi || 'Lompoe', deskripsi: body.deskripsi || '', foto: null };
...
console.log('Successfully consolidated all API endpoints into 4 Serverless Functions!');
