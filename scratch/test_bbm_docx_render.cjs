...
const outputPath = path.join(__dirname, 'test_bbm_output.docx');
fs.writeFileSync(outputPath, buf);

console.log('Successfully generated test Word file at', outputPath);

// Verify XML of generated output
const zipOut = new PizZip(buf);
const xmlOut = zipOut.files['word/document.xml'].asText();
const textOut = xmlOut.replace(/<[^>]+>/g, ' ');
console.log('--- GENERATED TEXT PREVIEW ---');
console.log(textOut.substring(textOut.indexOf('Diberikan alokasi Volume') - 600, textOut.indexOf('Diberikan alokasi Volume') + 400));
